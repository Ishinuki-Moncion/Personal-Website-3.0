...
> **AMENDMENT 2026-07-10 (v3.2r):** Step 9(g) as written is a **spec defect** — the lightbox frame is dominated by the law-protected UNFILTERED photograph, so its mean can never sit below the hero baseline 18.43 (the original v32o measured 51.29 against this criterion and shipped silently; the honest redo measured 51.35). Superseded gate, per the addendum's own §8b variance law ("isolate the effect under test via an evaluate_script A/B toggle… never single frames"): (a) same-frame scrim A/B — old 0.96 inline-forced vs shipped 0.985: shipped mean strictly below old (measured 51.53 → 51.35); (b) frame p50 ≤ the §8b hero mean 18.43 (measured p50 15 — the deep-black field holds); (c) the photo itself stays unfiltered. Frame-mean-vs-hero-mean comparisons are void for photo-dominated frames.
