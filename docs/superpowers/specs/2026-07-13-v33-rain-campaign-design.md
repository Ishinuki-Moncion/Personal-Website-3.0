...
- `runStagger` (`app.js:55–72`): per-char inline `style.transition`/`transitionDelay` + double-rAF kick → one `span.animate([...], { duration, delay: li*280 + ci*45, easing: var(--ease-out)'s cubic-bezier, fill: 'both' })` per char. Same offsets, same curves.
